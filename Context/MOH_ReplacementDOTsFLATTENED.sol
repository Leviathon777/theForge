 ...
 * Owner-only minting for gas fees - no purchase price
 * Specify exact token ID and recipient when minting
 */
contract MOH_ReplacementDOTs is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    // Distribution router address (set later when router is deployed)
    address public distributionRouter;
    ...
    ) ERC721("Medals Of Honor II", "MOH") Ownable(initialOwner) {}

    // ========== CORE MINTING ==========

    /**
     * @notice Forge a DOT with a specific token ID to a specific wallet
     * @dev Owner only - gas fees only, no purchase price
    ...
    /**
     * @notice Forge a replacement DOT linked to a stolen original
    ...
    function setDistributionRouter(address _router) external onlyOwner {
        distributionRouter = _router;
    }
