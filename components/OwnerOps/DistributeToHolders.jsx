...
/*
  ...
  const updateXdRiPWeights = async () => {
    if (!distributeContract || !signer) {
      setStatus("Distribution contract or signer not loaded.");
      return;
    }
    ...
    try {
      const investors = holdersInfo.map(holder => holder.address);
      const weights = holdersInfo.map(holder => holder.finalWeight);
  
      const distributeContractWithSigner = distributeContract.connect(signer);
      setStatus("Updating XdRiP weights...");
      const tx = await distributeContractWithSigner.updateXdRiPWeights(investors, weights);
      await tx.wait();
  
      setStatus("XdRiP weights updated successfully.");
    } catch (error) {
      console.error("Error updating XdRiP weights:", error);
      setStatus("Error updating XdRiP weights.");
    }
  };
  
  const handleDistributeRevenue = async () => {
    try {
      setStatus("Preparing for revenue distribution...");
  
      // Update XdRiP weights
      await updateXdRiPWeights();
  
      // Distribute revenue
      await distributeRevenue();
    } catch (error) {
      console.error("Error during revenue distribution:", error);
      setStatus("Error during revenue distribution.");
    }
  };
...
const DistributeRevShare = ({ onBack }) => {
  ...
  // Initialize contracts using window.ethereum
  useEffect(() => {
    const initializeContracts = async () => {
      try {
        if (!window.ethereum) {
        ...
        // (xdripInstance is not directly used here beyond initialization)
        
        const balance = await provider.getBalance(distributeCA_ABI.address);
        setContractBalance(ethers.utils.formatEther(balance));

        const currentPerc = await distributeInstance.distributionPercentage();
        setDistributePercentage(currentPerc.toNumber());

        setStatus("Contracts initialized successfully.");

        // Listen for the detailed event so you can verify distribution breakdown.
        distributeInstance.on("InvestorDistribution", (investor, baseWeight, bonus, finalWeight, shareSent) => {
          console.log(`InvestorDistribution: ${investor} | Base: ${baseWeight} | Bonus: ${bonus} | Final: ${finalWeight} | Share Sent: ${shareSent}`);
        });
      } catch (error) {
        console.error("Error initializing contracts:", error);
        setStatus("Error initializing contracts. See console for details.");
  ...
  // Fetch holders info and compute final weight using the same formula as your Reports:
  // finalWeight = base weight + base weight * (revshareBonus / 100)
  const fetchHoldersInfo = async () => {
    if (!mohContract) {
      setStatus("MOH contract not loaded.");
      return;
    ...
    try {
      ...
      // Process holders: calculate a "base weight" from medals, then update bonus and final weight.
      const holders = Object.entries(holderMap).map(([address, data]) => {
        // Use the ramp formula as in reports – this example takes the maximum medal weight:
        let totalWeight = 0;
        const tierCounts = medalOrder.reduce((counts, tier) => {
          counts[tier] = 0;
          return counts;
        ...
        // Calculate full ramps and leftover weight as in your reports code:
        const fullRamps = Math.min(...medalOrder.map((tier) => tierCounts[tier]));
      ...
      // For each holder, get their XdRiP balance and compute the revshare bonus in the same way as in Reports.
      const updatedHolders = [];
      for (const holder of holders) {
        const xdripBal = await fetchXDRIPBalance(holder.address);
  
        // Compute revshare bonus based on XdRiP balance (as a percentage of total supply of 1e9)
        ...
        console.log("xdrip percent", xdripPercentage);
  
        // Compute bonus as before (for example, 3.75)
const bonusAmount = holder.weight * (revshareBonus / 100);

const scaledBaseWeight = holder.weight * 100;
// The scaled final weight is the sum:
const scaledFinalWeight = scaledBaseWeight + scaledBonus;
...
updatedHolders.push({
  ...
  revshareBonus,
  bonusAmount,      // original bonus (e.g., 3.75) for reports display
  scaledBonus,      // scaled bonus (e.g., 375)
  finalWeight,      // displayed final weight (e.g., "28.75")
});
  ...
  // Update bonus weights on-chain and then trigger distribution.
  // (This function uses holder.bonusAmount values computed above.)
  const updateBonusWeightsAndDistribute = async () => {
    ...
    try {
      ...
      holdersInfo.forEach((holder) => {
        ...
        bonusValues.push(holder.scaledBonus); // Now bonus (375) preserves the 3.75 when descaled
      });
  ...
  console.log("Weights being sent:", scaledWeights);
        ...
        <div className={styles.updateMOHContract}>
          <p>
            ...
            <span
              className={styles.copyableAddress}
              onClick={() => {
                navigator.clipboard.writeText(currentMOHContract);
              }}
              title={`Click to copy ${currentMOHContract}`}
            >
              {shortenAddress(currentMOHContract)}
...
const DistributeRevShare = ({ onBack }) => {
  ...
  const [contractBalance, setContractBalance] = useState("0");
  const [depositAmount, setDepositAmount] = useState("");
  const [distributePercentage, setDistributePercentage] = useState(20);
  const [newPercentage, setNewPercentage] = useState("");
  const [distributeContract, setDistributeContract] = useState(null);
  const [mohContract, setMohContract] = useState(null);
  const [currentMOHContract, setCurrentMOHContract] = useState("Fetching...");
  ...
  useEffect(() => {
    const initializeContracts = async () => {
      try {
        ...
        const mohInstance = new ethers.Contract(mohCA_ABI.address, mohCA_ABI.abi, _signer);
        setMohContract(mohInstance);

        const distributeInstance = new ethers.Contract(distributeCA_ABI.address, distributeCA_ABI.abi, _signer);
        setDistributeContract(distributeInstance);

        const xdripInstance = new ethers.Contract(xdripCA_ABI.address, xdripCA_ABI.abi, _signer);
        setXdripContract(xdripInstance);

        const balance = await _provider.getBalance(distributeCA_ABI.address);
        setContractBalance(ethers.utils.formatEther(balance));
  ...
  const fetchHoldersInfo = async () => {
    ...
    try {
      ...
      // For each holder, compute their XdRiP balance and calculate bonus.
      const updatedHolders = [];
      for (const holder of holders) {
        const xdripBalance = await fetchXDRIPBalance(holder.address);
        const xdripPercentage = (xdripBalance / 1e9) * 100; // Assuming total supply is 1e9
        let revshareBonus = 0;
        if (xdripPercentage >= 1.48) {
          revshareBonus = 15;
        } else if (xdripPercentage >= 1.25) {
  ...
  // Updates on-chain final weights using the contract function updateFinalWeights()
  const updateFinalWeightsOnChain = async () => {
    if (!distributeContract || !signer) {
      setStatus("Distribution contract or signer not loaded.");
      return;
    }
    try {
      const investors = holdersInfo.map(holder => holder.address);
      const weights = holdersInfo.map(holder => holder.finalWeight);
      console.log("Updating final weights on chain for:", investors, weights);
      if (investors.length === 0 || weights.length === 0) {
        throw new Error("Arrays are empty.");
      }
      if (investors.length !== weights.length) {
        throw new Error("Mismatched arrays.");
      }
      const contractWithSigner = distributeContract.connect(signer);
      setStatus("Updating final weights on chain...");
      const tx = await contractWithSigner.updateFinalWeights(investors, weights, { gasLimit: 500000 });
      console.log("Final weights tx hash:", tx.hash);
      const receipt = await tx.wait();
      console.log("Final weights receipt:", receipt);
      setStatus("Final weights updated on chain successfully.");
    } catch (error) {
      console.error("Error updating final weights:", error);
      let errorMsg = "Error updating final weights.";
  ...
  // Distribute revenue: first update final weights, then call distributeRevenue on the contract
  const distributeRevenue = async () => {
    if (!distributeContract) {
      setStatus("Distribution contract not loaded.");
      return;
    ...
    try {
      setStatus("Distributing revenue...");
      await updateFinalWeightsOnChain();
      const tx = await distributeContract.distributeRevenue();
      ...
      console.error("Error distributing revenue:", error);
      let errorMsg = "Error distributing revenue.";
      if (error.data && error.data.message) {
        errorMsg = error.data.message;
      } else if (error.reason) {
        errorMsg = error.reason;
  ...
  const handleDepositFunds = async () => {
    ...
    try {
      setStatus("Depositing funds...");
      const _signer = distributeContract.provider.getSigner();
  ...
  async function fetchXDRIPTransferEvents() {
    try {
      console.log("Fetching XDRIP Transfer events with Web3...");
      const localXdRiPContract = new web3.eth.Contract(XdRiPContractABI, XdRiPContractAddress);
      const events = await localXdRiPContract.getPastEvents("Transfer", {
        fromBlock: 0,
        toBlock: "latest"
      });
      if (events.length === 0) {
