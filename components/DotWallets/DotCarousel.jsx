...
const Carousel = ({ medals = [] }) => {
  ...
  const handleMedalDetails = useCallback((medal) => {
    const videoPath = videoPaths[medal.metadata.name] || ""; // Use local video path
    console.log(`Loading video for ${medal.metadata.name}: ${videoPath}`); // Log video path
    ...
    <div className={styles.carousel} {...swipeHandlers}>
      ...
      <div className={styles.cards}>
        {displayedItems.map((item, index) => {
          const position = (index - currentIndex + 5) % 5;
          const cardClass = `${styles.card} ${styles[`position${position}`]}`;
          const videoSrc = videoPaths[item.metadata?.name] || ""; // Retrieve local path
          ...
          return (
            <div key={`${item.id}-${index}`} className={cardClass}>
              {item.isPlaceholder ? (
                ...
                !isImage(item.metadata.animation_url) ? (
                  <VideoPlayer
                    videoSrc={videoSrc} // Use local path for the video
                    isMuted={true}
                    hoverPlay={true}
                    autoPlay={true}
                    loop={true}
